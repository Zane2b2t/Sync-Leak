...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            Sync.textManager.drawStringWithShadow(this.currentString.getString() + Sync.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) SyncGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            Sync.textManager.drawStringWithShadow((this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.RED : "")) + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) SyncGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
